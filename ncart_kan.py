...
class GroupedKANLayer(nn.Module):
    """
    Grouped Kolmogorov-Arnold Network layer using B-splines
    """
    def __init__(self, input_dim, output_dim, num_groups=4, grid_size=5, spline_order=3):
        super(GroupedKANLayer, self).__init__()
        ...
        self.scale = nn.Parameter(torch.ones(num_groups, self.group_size, 1))
        self.shift = nn.Parameter(torch.zeros(num_groups, self.group_size, 1))
    ...
    def b_spline_basis(self, x, grid, order):
        ...
        """
        x = x.unsqueeze(-1)  # Add grid dimension
        grid = grid.unsqueeze(0).unsqueeze(0)  # Add batch and feature dimensions
        
        # Compute distances to grid points
        distances = torch.abs(x - grid)
        
        # Simple B-spline implementation (order 1 for simplicity)
        if order == 1:
            basis = torch.where(distances < 0.5, 1.0 - 2 * distances, torch.zeros_like(distances))
        else:
            # Higher order B-splines can be implemented here
            basis = torch.exp(-0.5 * (distances * 2) ** 2)  # Gaussian-like basis
    ...
    def forward(self, x):
        ...
        x_grouped = x_grouped.unsqueeze(-1) * self.scale + self.shift
        x_grouped = x_grouped.squeeze(-1)
...
class CART_KAN(nn.Module):
    def __init__(self, n_features, n_selected, n_out, n_trees, mask_type, kan_groups=4):
        super(CART_KAN, self).__init__()
        self.n_features = n_features
        self.n_selected = n_selected
        self.n_trees = n_trees
        self.n_out = n_out
        self.mask_type = mask_type
        self.kan_groups = kan_groups
    ...
    def get_gini(self, x):
        ...
        if self.projection:
            ...
            return ((1.-pos**2-neg**2)*projection_matrix.detach().cpu()).sum(-1).sum(0)  
...
class NCART_KAN(nn.Module):
    def __init__(self, n_features, n_selected, n_out, n_trees, n_layers, mask_type, kan_groups=4):
        super(NCART_KAN, self).__init__()
        ...
        if self.n_layers > 1:
            self.model_list = nn.ModuleList([CART_KAN(n_features,
                                                      n_selected,
                                                      n_features,
                                                      n_trees,
                                                      mask_type,
                                                      kan_groups) for _ in range(n_layers-1)])
        self.last_layer = CART_KAN(n_features,
                                   n_selected,
                                   n_out,
                                   n_trees,
                                   mask_type,
...
class NCARTClassifier_KAN():
    ...
    def fit(self, X, y, X_val=None, y_val=None):
        ...
        self.model = NCART_KAN(self.n_features,
                               self.n_selected,
                               self.n_out,
                               n_trees=self.n_trees,
                               n_layers=self.n_layers,
                               mask_type=self.mask_type,
                               kan_groups=self.kan_groups)
        self.to_device()
        ...
        for epoch in range(self.epochs):
            ...
            if X_val is not None:
                ...
                with torch.no_grad():
                    ...
                    if min_val_loss_idx + self.early_stop_round < epoch:
                        print("Validation loss has not improved for %d steps!" % self.early_stop_round)
